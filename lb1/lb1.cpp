...
bool lb1::proc4()
{
    ...
    for (const auto& cd : cds)
    {
        ...
        x_lambda_s.push_back(tmin + cd.mi * cd.dt);
        y_lambda_s.push_back(cd.lambda_s);
        y_lambda_s1.push_back(cd.lambda_s1);
        y_lambda_s2.push_back(cd.lambda_s2);


        x_lambda_s.push_back(tmin + (cd.mi + 1) * cd.dt);
        y_lambda_s.push_back(cd.lambda_s);
        y_lambda_s1.push_back(cd.lambda_s1);
        y_lambda_s2.push_back(cd.lambda_s2);
        ...
        if (cd.lambda_s1 > ymax)
            ymax = cd.lambda_s1;

        if (cd.lambda_s2 > ymax)
            ymax = cd.lambda_s2;

        if (cd.lambda_s < ymin)
            ymin = cd.lambda_s;

        if (cd.lambda_s1 < ymin)
            ymin = cd.lambda_s1;

        if (cd.lambda_s2 < ymin)
            ymin = cd.lambda_s2;
    }
